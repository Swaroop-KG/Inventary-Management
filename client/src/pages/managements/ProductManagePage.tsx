...
import bgman from '../auth/assets/manage.avif';
...
const ProductManagePage = () => {
  ...
  return (
    <>
      ...
      <div style={{ height:250,width:450,  backgroundImage: `url(${bgman})`,backgroundSize: 'cover',
...
const AddStockModal = ({ product }: { product: IProduct & { key: string } }) => {
  ...
  const [updateProduct] = useUpdateProductMutation(); // Add product update logic

  const onSubmit = async (data: FieldValues) => {
    const newStock = Number(data.stock) + product.stock;
    const payload = {
      stock: newStock,
      seller: product.seller,
    };
    ...
    try {
      ...
      if (res.statusCode === 200) {
        // If the new stock is 0, update the product status to "Out of Stock"
        if (newStock === 0) {
          await updateProduct({
            id: product.key,
            payload: { status: 'Out of Stock' }, // Assuming 'status' is a field for stock status
          }).unwrap();
        }
        toastMessage({ icon: 'success', text: res.message });
        reset();
...
const UpdateProductModal = ({ product }: { product: IProduct & { key: string } }) => {
  ...
  const onSubmit = async (data: FieldValues) => {
    ...
    try {
      ...
      if (res.statusCode === 200) {
        // If stock is updated to 0, set the status to "Out of Stock"
        if (updatedStock === 0) {
          await updateProduct({
            id: product.key,
            payload: { status: 'Out of Stock' }, // Assuming 'status' is a field for stock status
